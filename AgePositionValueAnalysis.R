...
# Get most recent valuation per player this season
...
player_analysis <- player_analysis %>%
  mutate(
    ...
    position = ifelse(is.na(position.x), "Unknown", position.x)
  )

# Summarize for heatmap
heatmap_data <- player_analysis %>%
  filter(!is.na(market_value_in_eur) & !is.na(position.x) & !is.na(age_bin)) %>%
  group_by(age_bin, position.x) %>%
  summarize(
...
all_combinations <- expand.grid(
  ...
  position.x = unique(player_analysis$position.x)
)

# Join with full combinations and fill missing with NA
heatmap_data <- all_combinations %>%
  left_join(heatmap_data, by = c("age_bin", "position.x")) %>%
  mutate(
...
# Plot the heatmap
heatmap_plot <- ggplot(heatmap_data, aes(x = age_bin, y = position.x, fill = avg_market_value_millions)) +
  geom_tile() +
...
# Save to file
ggsave("C:/Users/maxwe/OneDrive - The Pennsylvania State University/Downloads/heatmap.jpg", plot = heatmap_plot, width = 12, height = 6, dpi = 300)
